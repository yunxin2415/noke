...
const useResize = (props2, contentRef, resizeRef) => {
  const state2 = vue.reactive({
    resizedWidth: props2.inputBoxWidth
  });
  const inputWrapperStyle = vue.reactive({
    width: props2.inputBoxWidth
  });
  const resizeOperateStyle = vue.reactive({
    left: props2.inputBoxWidth,
    display: "initial"
  });
    ...
    return () => {
      ...
      return vue.createVNode("div", {
        ...
        "oninputBoxWidthChange": (width) => {
          if (props2.oninputBoxWidthChange) {
            props2.oninputBoxWidthChange(width);
          }
